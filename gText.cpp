...
uint8_t x2,y2;
	...
	x2 = x + columns * (EmergencyFontRead(FONT_FIXED_WIDTH)+1) -1;
	y2 = y + rows * (EmergencyFontRead(FONT_HEIGHT)+1) -1;
...
void gText::SpecialChar(uint8_t c)
{
	...
	if(c == '\n')
	{
		uint8_t height = EmergencyFontRead(FONT_HEIGHT);
...
char *ftoa(char *str, double val, int precision) {
		...
		*str++ = '.';
		...
		long decimal = abs((long) ((val - integer) * p[precision]));
...
void gText::CPrintFloat(float n, int16_t originx, int16_t originy){
...
#if 0
void gText::GetCharDims(char c, int16_t *w, int16_t *h) {
	uint8_t width = 0;
	uint8_t height = EmergencyFontRead(FONT_HEIGHT);
	uint8_t bytes = (height+7)/8;
	
	uint8_t firstChar = EmergencyFontRead(FONT_FIRST_CHAR);
	uint8_t charCount = EmergencyFontRead(FONT_CHAR_COUNT);
	...
	if( EmergencyFontRead(FONT_LENGTH) == 0 && EmergencyFontRead(FONT_LENGTH+1) == 0) {
    // zero length is flag indicating fixed width font (array does not contain width data entries)
	   width = EmergencyFontRead(FONT_FIXED_WIDTH); 
	   index = c*bytes*width+FONT_WIDTH_TABLE;
	}
	else{
	// variable width font, read width data, to get the index
	   for(uint8_t i=0; i<c; i++) {  
		 index += EmergencyFontRead(FONT_WIDTH_TABLE+i);
	   }
	   index = index*bytes+charCount+FONT_WIDTH_TABLE;
	   width = EmergencyFontRead(FONT_WIDTH_TABLE+c);
    }
...
uint8_t gText::EmergencyFontRead(uint16_t data) {
		return(FontRead(this->Font + data));
}
...
/*================== OLD FONT DRAWING ============================*/
	...
	for(uint8_t i=0; i<bytes; i++)	/* each vertical byte */
	{
		...
		{
			uint8_t data = EmergencyFontRead(index+page+j);
...
/*================== NEW FONT DRAWING ===================================*/
	...
	for(p = 0; p < pixels;)
	{
		...
		for(uint8_t j=0; j<width; j++) /* each column of font data */
		{
			...
			{
				fdata = EmergencyFontRead(index+page+j);
			...
			while((dp <= 7) && (tfp) < pixels)
			{
				...
				if((tfp & 7)== 7)
				{
					fdata = EmergencyFontRead(index+page+j+width);
...
void gText::CursorTo( uint8_t column, uint8_t row)
{
	...
	this->x = column * (EmergencyFontRead(FONT_FIXED_WIDTH)+1) + this->tarea.x1;
	this->y = row * (EmergencyFontRead(FONT_HEIGHT)+1) + this->tarea.y1;
...
void gText::CursorTo( int8_t column)
{
    ...
    if(column >= 0) 
	  this->x = column * (EmergencyFontRead(FONT_FIXED_WIDTH)+1) + this->tarea.x1;
   	  ...
   	  this->x -= column * (EmergencyFontRead(FONT_FIXED_WIDTH)+1);   	
...
uint8_t gText::CharWidth(uint8_t c)
{
    ...
    if(isFixedWidthFont(this->Font)){
		...
		//width = EmergencyFontRead(FONT_FIXED_WIDTH)+1;  // there is 1 pixel pad here
		width = EmergencyFontRead(FONT_FIXED_WIDTH)+1;  // there is 1 pixel pad here
	} 
    else{ 
	    // variable width font 
		uint8_t firstChar = EmergencyFontRead((uint16_t)FONT_FIRST_CHAR);
		uint8_t charCount = EmergencyFontRead((uint16_t)FONT_CHAR_COUNT);
	
		// read width data
		if(c >= firstChar && c < (firstChar+charCount)) {
			c -= firstChar;
			width = EmergencyFontRead((uint16_t)FONT_WIDTH_TABLE+c)+1;
		}
	}	
	return width;
}
