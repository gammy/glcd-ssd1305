...
byte colorBarData[] = { clrRed,clrOrange,clrYellow,clrGreen,clrCyan, clrBlue,clrLtMagenta};
...
void setup()
{
  ...
  if(GLCD.Height > 64) 
       ArduinoIcon =  ArduinoIcon96x32;  
  else if(GLCD.Height == 64)   
...
{  
  ...
  while(iter++ < 10){   // do 10 iterations
    GLCD.DrawRect(0, 0, GLCD.CenterX, GLCD.Bottom, clrOrange); // rectangle in left side of screen
    GLCD.DrawRoundRect(GLCD.CenterX + 2, 0, GLCD.CenterX - 3, GLCD.Bottom, 5, clrGreen);  // rounded rectangle around text area   
    for(int i=0; i < GLCD.Bottom; i += 4)
      GLCD.DrawLine(1,1, GLCD.CenterX-1,i, clrRed);  // draw lines from upper left down right side of rectangle  
...
void introScreen(){  
  GLCD.ForeColor = clrYellow;
  GLCD.DrawBitmap(ArduinoIcon, 32,0); //draw the bitmap at the given x,y position
  countdown(3);
  GLCD.ClearScreen();
  GLCD.SelectFont(Arial_14); // you can also make your own fonts, see playground for details   
  GLCD.GotoXY(10, 3);
  GLCD.ForeColor = clrRed;
  GLCD.print("GLCD ver ");
  GLCD.ForeColor = clrGreen;
  GLCD.print(GLCD_VERSION, DEC);
  GLCD.ForeColor = clrYellow;
  GLCD.DrawRoundRect(8,0,GLCD.Width-9,17, 5);  // rounded rectangle around text area   
  countdown(3);  
  GLCD.ClearScreen(); 
  colorScribble(5000, 7, colorBarData );  // run for 5 seconds
  //moveBall(6000); // kick ball for 6 seconds
  GLCD.SelectFont(System5x7, BLACK); // TODO Select font not yet modifed set ForeColor 
  GLCD.ForeColor = clrBlue;  
...
int ballDirectionX = 1;        // Y direction of the ball
  ...
  for(unsigned long start = millis();  millis() - start < duration; )
  {
    ...
    GLCD.FillCircle(ballX, ballY, ballR, clrOrange);
    delay(30 );
  }
}
 ...
 */
void colorScribble( const unsigned int duration, int nbrColors, byte palette[] )
{
  ...
  for(unsigned long start = millis();  millis() - start < duration; )
  {
    ...
    byte colorIndex =  t / (duration/ nbrColors);
    g_head_pos += tick ;
    ...
    byte x = fn_x( head ) ;
    byte y = fn_y( head ) ;
    GLCD.SetDot( x , y , palette[colorIndex]) ;
...
void colorBars(int waitMS)
{
  const byte nbrBars = sizeof(colorBarData) / sizeof(byte);
  byte barWidth = GLCD.Width / nbrBars;
  byte xPos = 0;
  
  for(int bar = 0; bar < nbrBars; bar++)
  {
      GLCD.SetPixels(xPos,  0,  xPos + barWidth, GLCD.Bottom, colorBarData[bar]);
      xPos += barWidth;
  }
  delay(waitMS);  
}
